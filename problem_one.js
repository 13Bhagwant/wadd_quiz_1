...
let isPangram = s => {
    let lowerCase = s.toLowerCase();//it will do all values of s to lowerCase

    return 'abcdefghijklmnopqrstuvwxyz'
        .split('')// is used as the separator, the string is split between each character.
        .filter(c => lowerCase.indexOf(c) === -1)// creates an array filled with all array elements that pass the c function .
